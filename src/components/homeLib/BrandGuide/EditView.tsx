...
export default class EditView extends React.Component<EditViewProps> {
  ...
  getFormItem = (info, index) => {
    ...
    return(
      <Card key={index} title={`品牌${index + 1}`} style={{ marginTop: '20px' }}>
            <Form.Item label={`品牌${index + 1}名称`}>
              {getFieldDecorator(`contentList[${index}].title`, {
                rules: [{ required: true, message: '必填项' }],
                initialValue: info.title
              })(<Input />)}
            </Form.Item>
            <Form.Item label={`品牌${index + 1}图片`}>
              {getFieldDecorator(`contentList[${index}].iconUrl`, {
                initialValue: info.iconUrl
              })(<UploadImg width={50} height={50} />)}
              <Alert
                message="格式为png或gif，尺寸50px*50px"
                type="info"
                showIcon
              />
            </Form.Item>

            <Form.Item label={`品牌${index + 1}打点值`}>
