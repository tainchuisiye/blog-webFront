...
> {
  ...
  handleSelectChange = (val, type) => {
    ...
    while (params.length < val) {
      params.push({} as ParamsItem);
    }
    if (params.length > val) {
      params = params.slice(0, val);
    }
    this.setState({
      [type]: params
    });
  ...
  render() {
    ...
    if (value) {
      try {
        let json: any = value.split('taoqicar://mall/itemDetail?param=')[1];
        json = JSON.parse(decodeURIComponent(json));
        id = json .id;
        track = json.trackValue;
      } catch (error) {
        //
      }
    }
        ...
        <Form>
          <Form.Item {...formItemLayout} label="链接类型">
            {getFieldDecorator(`type`, {
              initialValue: this.props.isOnlyCars ? 1 : type
            })(
              <Radio.Group   onChange={this.resetForm}>
                <Radio disabled={this.props.isOnlyCars} value={0}>商品列表</Radio>
                <Radio value={1}>商品详情</Radio>
                <Radio disabled={this.props.isOnlyCars} value={2}>web容器</Radio>
              </Radio.Group>
            )}
          </Form.Item>
          {type === 0 && (
            <div>
              ...
              {params1.map((arr, i) => (
                <Form.Item {...formItemLayout} label={'参数' + (i + 1)} key={i}>
            ...
            <div>
              <Form.Item {...formItemLayout} label="参数名">
                {getFieldDecorator(`params2[0].type`, {
                  initialValue: this.props.isOnlyCars ? paramKeyArray2[0].name : params2[0].type
                })(
                  <Select placeholder="请选择参数类型">
                ...
                <Form.Item {...formItemLayout}>
                  {getFieldDecorator(`params2[0].value`, {
                    initialValue: params2[0].value || id
                  })(<Input placeholder="请输入参数名" style={inputStyle}/>)}
                </Form.Item>
              </Form.Item>
            </div>
